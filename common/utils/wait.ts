 ...
 * @param attempts - The number of reloads before failing the test
 */
export const waitForComboboxOptions = async (
  ...
  for (let i = 0; i < attempts; i++) {
    ...
    if (await iframe.getByRole('option', { name: 'No results found' }).isVisible({ timeout: 1_000 })) {
      ...
      console.log(`🔃 '${name}' options not loaded on attempt ${i + 1}/${attempts}, reloading...`);
      await page.reload();
      await setupFn?.();
    } else {
      console.log(`✅ '${name}' options loaded on attempt ${i + 1}/${attempts}`);
      // Close the combobox before returning to avoid blocking the next test
      await combobox.click();
      return;
    }
  }
  throw new Error(`❌ '${name}' options not loaded after ${attempts} attempts`);
};
